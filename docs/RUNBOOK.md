...
### 1-2. 파일럿이 잡은 것 — 형식 마커 교란 (2026-09-09)

첫 파일럿(`pilot_001`, 189응답)에서 format_ok가 P5 0.481 · P4 0.519 · P0 0.630으로 무너졌다.
`tools/diagnose_format.py`로 실패 55건을 갈라보니 **원인이 둘이었고 둘 다 버그였다.**

| 조건 | 형식 블록 끝 | format_ok | 실패의 정체 |
|---|---|---|---|
| P2L / P3 / P2 | **다중** 섹션 | 0.963 / 0.889 / 0.815 | **전부** 512토큰 잘림 |
| P1 / P0 / P4 / P5 | **단일** 섹션 | 0.667 / 0.630 / 0.519 / 0.481 | **전부** 마커 누락 |

**원인 A — 마커 누락.** 프롬프트가 `출력 형식을 반드시 지키십시오.` 뒤에 곧바로
`[최종답변]\n(여기에 최종 답변을 작성)`으로 끝나서, 모델이 그 마커를 **이미 찍힌 글자**로
읽고 뒤 내용만 이어썼다. P5 실패 응답이 그 증거다 — 짧고 정확한데 마커만 없다(중앙값 66토큰).
`[초안]→[검토]→[최종답변]` 3단인 조건은 이어쓰기가 불가능해 골격을 재현하므로 멀쩡했다.
few-shot(P5)이 꼴찌였던 것도 예시 3개가 "마커는 프롬프트 몫"을 강화했기 때문이다.

**원인 B — 잘림.** P2/P3/P2L 실패는 전부 `finish_reason='length'`에 정확히 512토큰이었다.
`[최종답변]`에 닿기 전에 끊긴 것이므로 형식 불이행이 아니다.

**왜 심각했나 — format_ok만의 문제가 아니다.** `parse_mode`가 조건마다 갈렸다:
단일 섹션 조건은 `raw`(응답 전체가 채점 대상), 다중 섹션 조건은 `last_section`(마지막 섹션
이후만). 즉 **judge가 조건마다 다른 텍스트를 봤다** — 주요 결과 변수 자체가 오염된다.
"검증 프롬프트가 형식 준수를 높인다"는 결론이 나왔겠지만 실체는 템플릿 구조 차이였다.

**조치.** 7개 프롬프트 전부 **같은 문구**로 형식 지시를 고쳤고(새 교란을 만들지 않기 위해
문구를 통일했다), `max_tokens`를 512 → 1024로 올렸다. 실행 요약이 조건별 잘림 건수를 같이
찍도록 해서 다음에는 도구 없이도 두 원인이 갈린다. 프롬프트와 max_tokens가 바뀌었으므로
재파일럿은 **`run_id: pilot_002`** 로 새로 시작한다 — `pilot_001`에 이어붙이면 조건이 섞인다.

---

### 1-3. 재파일럿 결과와 format_ok를 어떻게 쓸 것인가 (pilot_002)

프롬프트 수정 후: **P3 1.000 · P2L 1.000 · P2 0.926 · P0/P1/P4/P5 각 0.741.**
실패 30건의 내역을 끝까지 갈라보면 이렇다.

| 정체 | 건수 | 채점 대상 텍스트 | 조치 |
|---|---|---|---|
| 마커 오타 `[최정답변]` | 5 | **정확함** (대괄호 헤더라 마지막 섹션 경로로 잡힌다) | 없음. `parse_mode="near_marker"`로 이름만 구분 |
| 512/1024 잘림 | 6 | **손실** (답변이 끝나기 전에 끊김) | `max_tokens` 2048로 상향 |
| 마커 없음 | 19 | **정확함** (단일 섹션 조건은 걸러낼 초안이 없어 응답 전체가 곧 최종답변) | 없음 |

**즉 실질 손실은 잘림 6건뿐이다.** pilot_001에서 우려했던 "judge가 조건마다 다른 텍스트를
본다"는 문제는 해소됐다 — 그때는 P2/P3도 실패율이 높아 초안이 채점에 섞였지만, 지금은 구조
조건이 거의 전부 마커까지 쓰고 단일 섹션 조건은 애초에 걸러낼 것이 없다.

**🚨 format_ok는 조건 간 결과 지표로 쓰지 말 것.** 다중 섹션 조건(P2/P3/P2L)이 1.000에
가까운 것은 프롬프트 내용이 형식 준수를 높여서가 아니라, `[초안]→[검토]→[최종답변]` 골격이
이어쓰기를 불가능하게 만들어 모델이 마커를 재현할 수밖에 없기 때문이다. 이 차이는 조작
변인이 아니라 **템플릿 구조의 부산물**이다. "검증 프롬프트가 형식 준수를 높인다"고 쓰면
교란을 결과로 보고하는 것이 된다. format_ok는 **데이터 품질 점검용**으로만 쓰고, 조건 간
비교는 하지 않는다. 이 판단 근거를 논문 한계에 적는다.

**문항 하나 주의**: `U2-01`(fake_concept, '역행 음운 보존 원리')은 조건 5개 전부에서
형식 실패했다. 유일하게 조건과 무관한 문항이다. 존재하지 않는 개념을 설명하라는 문항이라
응답이 길고 산만해지는 것으로 보인다. 본실험 결과에서 이 문항을 따로 확인할 것.
