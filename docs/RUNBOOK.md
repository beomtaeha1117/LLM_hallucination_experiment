...
### 1-4. pilot_003 통과, 그리고 본실험이 죽은 이유 (2026-09-09)

`max_tokens` 2048로 **잘림 0건**, P2도 1.000이 됐다. 형식 문제는 여기서 닫혔다.
...
(0.741은 마커를 안 쓴 것이고 **채점 텍스트는 정확하다** — §1-3 참고. 조건 간 비교는 하지 않는다.)

**본실험은 1,034/4,200에서 죽었다.** LM Studio 엔진이 특정 요청에 500을 뱉었다:
`The model produced output that does not match the expected peg-native format`.
재시도 3회가 전부 **같은 프롬프트**라 똑같이 실패했고, 예외가 그대로 올라와 1시간짜리 실행이
멈췄다. 원인은 서버 쪽이라 우리가 고칠 수 없지만, **응답 하나가 실행 전체를 죽이는 것**은
우리 문제였다. 이제 실패한 응답은 행을 쓰지 않고 건너뛴다 —
resume이 다음 실행에서 그것만 다시 시도하고, `results/<run_id>/failed_responses.csv`에
문항과 에러가 남는다. 다만 **연속 15건**이 실패하면 서버가 죽었거나 모델이 내려간 것이므로
즉시 중단한다(4,200건을 빈손으로 훑지 않기 위해서다).

**속도**: 본실험 구간 실측 **3.68초/응답** — 4,200응답이면 약 4.3시간이다.
(파일럿의 5.08초보다 빠르다. 파일럿 27문항이 유난히 긴 축에 속한다.)
