...
| **thinking 끄기** | 어느 후보가 수락되고 토큰이 줄었는지 | 어느 것도 확실하지 않으면 **config의 thinking을 지우고 "추론 모드를 끄지 못했다"를 한계에 쓴다.** 되는 척하지 말 것 |
...
### 3-1. 보류 표현 사전 (`data/abstention_lexicon.txt`)
🚨 **P0·P2 응답을 먼저 보고 만든 뒤, 그 다음에 P1·P3에만 나오는 표현을 추가한다.**
P1·P3 프롬프트는 "확인할 수 없습니다"를 직접 지정하므로, 사전을 그 표현에 맞춰 만들면
P0·P2의 보류가 과소 탐지되어 **조건 간 차이가 인위적으로 부풀려진다.**
