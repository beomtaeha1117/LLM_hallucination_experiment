...
문항 검증은 **113/200**까지 왔고, 더 중요한 것은 **답변불가(N) 100건 중 98건이 끝났다**는 것이다
— 채점 기준이 틀리면 -2인 구간이 여기다. 남은 87건은 대부분 answerable(Y) 계열이다.
...
| false_premise | 35 | **33 완료** (U3-E02 미조회 / U3-P06 교체권고) |
| unknowable | 15 | **완료** (구조적 판정) |
...
4. 파일럿 결과로:
   - 보류 표현 사전(`data/abstention_lexicon.txt`, `data/false_premise_lexicon.txt`) 구축 후 **동결**
   - judge 모델 선정 (κ 기준)
...
- **보류 표현 사전은 P0·P2 응답으로 먼저 만든다.** P1·P3에만 "확인할 수 없습니다"라는 표현이 지정되어 있어서, 사전을 그 표현에 맞추면 조건 간 차이가 인위적으로 부풀려진다. **P5(few-shot)도 배제 대상이다** — P5의 예시 3개가 그 "확인할 수 없습니다"라는 P1·P3 전용 문구를 그대로 쓰고 있어서 동일한 오염원이다. `data/abstention_lexicon.txt`(순수 보류)와 `data/false_premise_lexicon.txt`(잘못된 전제 지적) 둘 다 이 규칙을 따른다.
...
## 9. 다음 세션이 먼저 할 일 (2026-09-08 기준)

### A. 맥 스튜디오에 앉으면 제일 먼저 — 10분
```bash
python tools/probe_lmstudio.py --model <LM Studio에 뜨는 모델 id>
```
`thinking` 파라미터명, seed 반영 여부, 실제 tok/s와 그로부터 계산한 소요 시간을
**서버에 직접 물어본다.** 이 셋이 파일럿 진입의 진짜 관문이다.
셋 다 코드가 알 수 없는 것이고, 프로브는 추측하지 않고 되는 것과 안 되는 것만 출력한다.
출력을 그대로 가져오면 config와 문서에 반영한다.

**seed가 반영되지 않으면 실험 설계를 다시 봐야 한다** — 반복 3회가 seed로만 갈리는 구조다.

### B. 문항 2건 결정 (연구자 승인 필요)
- `U3-P06`(의병장 정문수) 교체 여부 — 부존재를 증명할 수 없는 문항이다.
- `U3-E02`(세종 1445 해양 연구 기관) 조회.

### C. 남은 문항 87건
전부 answerable(Y) 계열이라 위험도가 낮다. 정답이 틀리면 정확도 지표가 흔들리지만,
"없는 것을 있다고 표시"하는 -2 조건에는 해당하지 않는다.
**hard_factual 50건**이 그중 제일 크다.

### D. ⏳ 시한이 있는 문항
`U4-F01`은 2027학년도 수능(**2026년 11월 시행**) 전에 실험을 끝내야 답변불가로 유지된다.

---

## 10. 채점 편향 결함 4건 — 2026-09-08 수정 완료 ✅

프롬프트 7종을 정독하다가 **채점 쪽에서** 발견한 것들이다. 프롬프트 자체는 건전했다
(P2에 보류 지시가 섞여 있지 않아 2×2는 성립한다). 문제는 채점이었다.
...
| # | 증상 | 부풀리는 지표 | 피해 조건 |
|---|---|---|---|
| 13 | 헤지가 섞인 **정답**이 과잉보류로 찍힘 | 과잉보류율 | **P1, P3** |
| 14 | `[최종답변]` 마커가 없으면 초안 전체가 채점 대상 | 환각률 | **P2, P3** |

헤지하라고 **지시받은** 조건이 P1·P3고, 초안/검토 섹션을 내는 조건이 P2·P3·P2L이다.
즉 **2×2의 네 칸 중 P0만 멀쩡하고 나머지 셋이 선택적으로 불리해진다.**
이대로 돌렸으면 "어떤 프롬프트도 baseline보다 낫지 않다"는 결과가 나올 수 있었고,
그게 데이터인지 버그인지 구분할 방법이 없었다.

### 재현했던 사례
```
"정확한 수치는 확인되지 않았지만, 약 100,449 km²입니다."  -> ABSTAIN (rule)   # 정답인데 과잉보류
"약 100,449 km²입니다."                                  -> CORRECT (match)
```
```
[초안] ...응시시간이 30% 증가한다고 보고했습니다.
[검토] 확인해 보니 해당 논문을 찾을 수 없습니다.
결론적으로 확인할 수 없습니다.
   -> format_ok=False, response_final에 "30% 증가"가 남아 환각으로 계산됨
      (모델이 스스로 철회한 날조인데)
```

### 고친 방식
- **#13**: Y분기를 N분기와 대칭으로. `rule_abstain`이어도 **정답이 매칭되면 judge로 넘긴다.**
  보류 표현만 있고 정답이 없을 때만 규칙으로 ABSTAIN 확정.
- **#14**: 마커가 없으면 **마지막 대괄호 섹션 이후**를 쓴다. 헤더가 아예 없으면 전체.
  어느 경로였는지 `parse_mode`(`marker`/`last_section`/`raw`)에 기록하고,
  **조건별 `format_ok_rate`와 `parse_mode` 분포를 `summary.csv`에 남긴다** —
  조건 간 형식 준수율 차이 자체가 공시해야 할 교란이다.
- **#15/#16**: `data/false_premise_lexicon.txt`를 새로 만들어 전제 지적 표현을
  보류 사전에서 **옮겼다**(복사가 아니라). `response_kind` 진단 컬럼으로
  `abstain` / `false_premise_correction` / `answer` / `judge_error`를 구분한다.
  **최종 라벨 4종의 정의는 건드리지 않았다** — 라벨을 바꾸면 지표 6종과 통계 모형이 흔들린다.

### 함께 고친 것 (수정 검증 중에 발견)
- **MockJudge가 같은 편향을 갖고 있었다.** `_keyword_label`이 보류 사전을 정답 매칭보다
  먼저 봐서, 실제 judge라면 CORRECT로 판정할 헤지+정답을 ABSTAIN으로 찍었다.
  mock 산출물을 보고 잘못된 결론을 내릴 수 있어 순서를 뒤집었다.
- **`response_kind`가 라벨과 모순되는 행이 생겼다.** 헤지+정답이 `label=CORRECT`인데
  `kind=abstain`으로 나왔다. 답변가능 문항에서 CORRECT는 "실제로 답했고 맞았다"는 뜻이므로
  `answer`가 되도록 `answerable`을 분류에 반영했다.

### 검증 (전부 직접 실행)
```
pytest tests/ -q                     -> 19 passed (기존 12 + 신규 7)
python tools/validate_questions.py   -> 200행, Y=100/N=100
python run_all.py --config config_pilot.yaml -> mock 전 구간 통과
```

### 남은 설계 한계 (고칠 수 없는 것 — 논문에 명시할 것)
- **verification 요인이 출력 구조와 얽혀 있다.** P0·P1은 1섹션, P2·P3는 3섹션이다.
  P2L이 이걸 잡으려 하지만 P2L은 2×2 밖이고, **길이 공변량은 "길이"를 잡을 뿐
  "구조"를 못 잡는다.** 설계상 분리 불가능하므로 한계로 쓴다.
- **P4가 "환각(hallucination)"이라는 학술어를 쓴다.** 모델이 이 단어를 아는지에 따라
  효과가 갈린다. 파일럿에서 확인하고 필요하면 P4a/P4b로 분리한다.
