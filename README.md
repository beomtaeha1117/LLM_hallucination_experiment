...
- **[docs/SETUP.md](docs/SETUP.md)** — 맥에 아무것도 없는 상태에서 실험을 돌릴 수 있는 데까지.
  터미널 여는 법, 파이썬 설치, GitHub에서 내려받기(ZIP 포함), LM Studio와 모델 설치까지 전부.
- **[docs/RUNBOOK.md](docs/RUNBOOK.md)** — 설치가 끝난 뒤의 실행 순서.
  프로브 → 파일럿 → 보류 사전 동결·judge 선정 → 본실험 → 인간 검증.
- **[docs/STATUS.md](docs/STATUS.md)** — 지금 어디까지 왔는지, 그리고 밟으면 안 되는 함정들.
...
`Abstention F1`은 **보조 지표다** (2026-09-08 변경). 답변가능·불가가 반반인 이 문항 세트에서는
**전부 보류해도 F1이 약 0.67**이 나오고, 보류하지 않은 답이 맞았는지는 이 수치에 반영되지 않는다.
